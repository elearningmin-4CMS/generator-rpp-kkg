...
const App: React.FC = () => {
  ...
  const generateRPP = async () => {
    ...
    setLoading(true);
    try {
      const apiKey = import.meta.env.VITE_GROQ_API_KEY || (window as any).process?.env?.GROQ_API_KEY;
      if (!apiKey) {
        throw new Error("Groq API Key tidak ditemukan. Pastikan VITE_GROQ_API_KEY sudah diatur di environment variables.");
      }
      ...
      const prompt = `
        ...
        Berikan JSON murni (tanpa markdown wrapper) dengan struktur berikut:
        {
        ...
        PENTING: Buat array "lkpd" sesuai jumlah pertemuan (${formData.jumlahPertemuan}). Setiap item harus disesuaikan dengan model dan metode pertemuan masing-masing.
      ...
      const chatCompletion = await groq.chat.completions.create({
        messages: [
          {
            role: "system",
            content: "Anda adalah asisten ahli kurikulum madrasah (MI) yang membantu guru menyusun RPP berkualitas tinggi dalam format JSON."
          },
          {
            role: "user",
            content: prompt,
  ...
  const handleDownloadWord = () => {
    ...
    const html = `
      ...
      <head><meta charset='utf-8'><title>RPP KKG Jatinagara</title>${styles}</head>
      <body>${content}</body>
      </html>
    `;
                ...
                <div id="rpp-printable" className="print-page border-2 border-green-900 p-8 bg-white text-[11pt] leading-tight shadow-sm">
                  ...
                  <table className="w-full border-collapse mb-1">
                    ...
                    <tbody>
                      ...
                      <tr><td className="border border-slate-300 p-1 bg-slate-50 font-semibold">Strategi Pedagogis</td><td className="border border-slate-300 p-1">{(formData.pertemuanDetails || []).map((p, i) => `P${i+1}: ${p.model} (${(p.methods || []).join(', ')})`).join('; ')}</td></tr>
                      <tr><td className="border border-slate-300 p-1 bg-slate-50 font-semibold">Kemitraan & Lingkungan</td><td className="border border-slate-300 p-1">{generated.kemitraan}, {generated.lingkungan}</td></tr>
